{
  config_exporters = { optionalAttrs, ... }: [
    ...
    (config: {
      ec2target = optionalAttrs (config.deployment.targetEnv == "ec2-target") {
        inherit (config.deployment.ec2) target privateKey;
      };
    })
  ...
  resources = args@{ evalResources, zipAttrs, resourcesByType, ... }: {
    ...
    ec2LaunchTemplates = evalResources ./aws-ec2-launch-template.nix (zipAttrs resourcesByType.ec2LaunchTemplates or [ ]);
    awsSpotFleets = evalResources ./spot-fleet.nix (zipAttrs resourcesByType.awsSpotFleets or [ ]);
